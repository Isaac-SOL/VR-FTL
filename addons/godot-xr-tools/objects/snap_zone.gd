...
func drop_object() -> void:
	...
	# Allow the objects to collide again
	var physics_parent := get_parent() as PhysicsBody3D
	var physics_picked_up_object := picked_up_object as PhysicsBody3D
	if physics_parent and physics_picked_up_object:
		physics_parent.remove_collision_exception_with(physics_picked_up_object)
		physics_picked_up_object.remove_collision_exception_with(physics_parent)
...
func pick_up_object(target: Node3D) -> void:
	...
	# If object picked up then remove collisions and emit signal
	if is_instance_valid(picked_up_object):
		var physics_parent := get_parent() as PhysicsBody3D
		var physics_picked_up_object := picked_up_object as PhysicsBody3D
		if physics_parent and physics_picked_up_object:
			physics_parent.add_collision_exception_with(physics_picked_up_object)
			physics_picked_up_object.add_collision_exception_with(physics_parent)
